...
public class HistoryTests : XunitTestBase
{
    ...
    {
        Result<History> result = await YahooQuotes.GetHistoryAsync("ABM", "", "1d", TestContext.Current.CancellationToken);
        History history = result.Value;
    ...
    {
        Result<History> result = await YahooQuotes.GetHistoryAsync("LRCX", "", "1d", TestContext.Current.CancellationToken);
        History history = result.Value;
